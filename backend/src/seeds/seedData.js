...
export const seedDatabase = async () => {
  try {
    ...
    console.log('🌱 Seeding initial demo users, scenarios, and compliance records...');
    ...
    const trainee1 = await User.create({
      ...
      hasCompletedOnboarding: false, // Alex gets the first-time onboarding overlay
    });
    ...
    console.log('✅ Seeded 4 demo accounts: trainee1, trainee2, supervisor1, admin1');

    // 2. Create Default 360 Scenario
    ...
    console.log(`✅ Seeded scenario: ${scenario.title} (${scenario.code}) with 5 hazards and 4 quiz questions.`);

    // 3. Seed sample compliance records for Supervisor Dashboard demo
    await ComplianceRecord.create([
      ...
      },
      {
        ...
        completedAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000), // 3 days ago baseline
      },
    ]);

    console.log('✅ Seeded demo compliance records for supervisor analytics.');
  } catch (error) {
